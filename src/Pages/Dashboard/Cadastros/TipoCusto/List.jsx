...
import TipoCustoModel from '../../../../Models/TipoCusto';

export default function TipoCustoList() {

	let [tipoCustos, setTipoCustos] = useState(new TipoCustoModel());
	...
	const showModalExcluir = function(e, tipo){
        ...
        swal.fire({
            title: 'Exclusão de tipo de custo',
        ...
        .then((response) => {
            ...
            if(response.isConfirmed){                
				...
				setShowLoader(true);
                axios.get(`api/tipo-custos/delete/${tipo.id}`)
                    .then((response) => { 
						setShowLoader(false);
						setTipoCustos(response.data);
                        NotificationManager.success('Registro excluído com sucesso.', 'Tipo de Custo');
                    })
                    .catch((error) => {
						setShowLoader(true);
	...
	const renderTipoCustos = function(){
							...
							return (
								<>
									<div className="d-flex justify-center">
										<Link to={`/app/tipo-custos-create/${data.id}`}>
											<Button
												type="normal"
			...
			<>
				<div className="project-nav justify-content-end">
					<div className="d-flex align-items-center">               
						<Link to={`/app/tipo-custos-create`}>
							<Button
								className='ml-1'
								text='Novo Tipo de Custo'
								type="normal"
								icon='fas fa-plus'
							/>
						</Link>
	...
	return(
		<>		
			<Content titulo="Lista de Tipos de custos">
				{ renderPage() }
			</Content>
		</>		
	)
